...
data_path = './data/component1_imputed.csv'
model_path = './models/component1.joblib'
...
selected_features = ['AL', 'CA', 'FTBN', 'ZN', 'B']
...
ftbn_default = df['FTBN'].mean()
zn_default = df['ZN'].mean()
b_default = df['B'].mean()
...
ftbn = st.number_input('FTBN', min_value=0.0, max_value=50000.0, step=0.1, value=ftbn_default)
zn = st.number_input('ZN', min_value=0.0, max_value=50000.0, step=0.1, value=zn_default)
b = st.number_input('B', min_value=0.0, max_value=50000.0, step=0.1, value=b_default)

if st.button('예측'):
    # Prepare the input data
    input_data = np.array([[al, ca, ftbn, zn, b]])
    # Make prediction
